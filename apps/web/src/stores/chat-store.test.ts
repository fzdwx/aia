...
describe("chat store submitTurn", () => {
  ...
  test("switchModel updates session-scoped settings instead of global provider", async () => {
    ...
    globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "/api/providers") {
        return new Response(
          JSON.stringify({ name: "openai", model: "gpt-5", connected: true }),
          { status: 200, headers: { "Content-Type": "application/json" } }
        )
      }
    ...
    useChatStore.getState().switchModel("openai", "gpt-5", "high")
