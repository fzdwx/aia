...
- 最新 `Question` host-tool 收口：`Question` 已不再走 `agent-runtime` 内部的专属等待/恢复状态机。当前实现改成由 `apps/agent-server` 以普通工具形式注册 `Question`：tool 负责生成结构化 `QuestionRequest` 并直接等待 server 侧 waiter，server 则负责 `question_requested/question_resolved` append-only 持久化、pending 查询与 `/api/session/question` 控制面。这样 runtime 不再需要理解 `Question` 业务语义，只把它当成普通工具调用处理。
- 最新 `Question` 协议再收口：单个问题项现在只保留 `question` 作为展示文案，不再额外携带 `header`；推荐项也从 `recommended_option_ids[]` 收紧为单值 `recommended_option_id`，避免模型/UI 两侧继续承接多余形态。与此同时，`PUT /api/session/question` 现在只负责落 `question_resolved` 并唤醒等待中的 server-side question tool，而不再伪造一条给 runtime 恢复专用的 `tool_result(Question)`。
