...
- 最新 bootstrap 收口：`apps/agent-server` 现已补成 `lib + bin` 双入口，外部嵌入方不再需要手写 `SessionManagerConfig` 或复制启动装配，而是可以直接走 `bootstrap_state_with_options(ServerBootstrapOptions)` 把 `registry_path/workspace_root/user_agent/system_prompt/runtime_hooks` 一次性注入到共享 control-plane façade。
