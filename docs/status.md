...
- 最新 `Question` 结果收口：`builtin-tools::question` 现在除了发起 `PendingToolRequest(kind=question)` 之外，也统一负责 `QuestionResult` 的 request_id / question_id / option_id 校验，以及最终 `tool_result(Question)` 的构造；`apps/agent-server` 因此退回到桥接与控制面职责，只负责 pending question 解码、`question_requested/question_resolved` 事实落盘和恢复原 turn，不再自行解释回答结果的具体语义。
- 最新交互能力收口：`agent-core::SessionInteractionCapabilities` 已开始从 `supports_question_tool` 过渡到更通用的 `supported_interaction_kinds`，`ToolDefinition` / `Tool` 也新增了 `interactive_kind` 元数据；当前 `Question` 已声明 `interactive_kind=question`，`agent-runtime.visible_tools()` 会优先按 interactive kind 过滤工具可见性，而旧的 `supports_question_tool` 仍保留为 question 的兼容映射，避免后续新增交互型工具时继续堆出第二个、第三个专用布尔开关。
