...
- 最新 provider 持久化收口：provider 注册表现在也已并入 `agent-store` 的 SQLite，由 `provider_registry` 表承接 `ProviderRegistry` JSON 快照；`apps/agent-server` 启动时直接从 `.aia/store.sqlite3` 读取 provider 状态，provider CRUD 也先写 SQLite 再更新 runtime / snapshot，不再额外依赖 `.aia/providers.json`。
...
- 完成 provider 注册表 SQLite 持久化：`agent-store` 新增 `provider_registry` 表与同步/异步读写 API，`apps/agent-server` bootstrap 与 provider CRUD 已改为直接读写 `.aia/store.sqlite3`，不再继续依赖 `.aia/providers.json`
