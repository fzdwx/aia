...
- 最新 trace dashboard 收口：为了支撑 overview 的失败态 / token / 行变更 / session 活跃指标，`agent-store` 现已把 `session_id` 从 runtime → trace 记录链路贯通到 SQLite，并把 `llm_trace_loops` 再扩展为携带 `session_id`、估算成本、代码增删行的 loop 级分析记录；`/api/traces/dashboard` 会先只对 `llm_trace_dirty_loops` 标记的脏 trace 做 loop reconciliation，再按时间窗聚合出失败/部分失败计数、`input/output/cache` token 趋势与年度活跃热力图，前端因此无需继续在浏览器里临时拼 analytics 数据，也避免每次 overview 读都重刷全部 loop。
