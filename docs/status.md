...
- 最新 provider 路由收口：`apps/agent-server/src/routes/provider.rs` 已按 `routes/channel` 的样式拆成目录模块；当前 `routes/provider/mod.rs` 仅保留 façade 导出，`dto.rs` 承接 HTTP 请求/响应类型。`ProviderRouteService` 已改为在 bootstrap 阶段装配并存入 `AppState` 的长生命周期服务对象，请求期不再重复 `new` 借用上下文；`handlers.rs` 里的 Axum 入口只保留最薄的参数提取与返回包装，而 provider 专属的纯映射/纯投影 helper 已直接内聚回 `handlers.rs`，避免无状态 helper 过度包装成对象，也避免再为它们单独占文件。
