...
- 最新飞书修复进展：私聊回复路径已从“总是 reply 当前消息”收紧为“私聊按 `chat_id` 直发、群聊按需 reply/thread”；SSE 现已补齐 `current_turn_started` 事件，前端对外部 channel 触发的当前执行态支持即时显示与流式恢复，不再依赖手动刷新；飞书消息进入处理中后会先加 `Typing` 表情，结束后再移除；同时飞书回包也已升级为交互卡片：先发可更新卡片，再按流事件节流刷新，最后收口为更完整的结果卡片。
...
- 完成飞书流式交互卡片：收到消息后会先发送可更新的 interactive card，随后根据 `thinking/text/tool` 流事件节流刷新卡片内容；最终卡片会收口为带标题、用户消息、可折叠思考过程、工具状态与耗时 footer 的更清晰布局
