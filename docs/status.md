...
- 最新前端配置台收口：`apps/web` 的 `Settings` 视图已回收为与 `Channels` 同类的 master-detail 结构，sidebar 直接列出 provider 与 “New provider” 入口，右侧仅保留当前 provider 的 endpoint/model catalog 管理，不再重复堆叠 provider 摘要卡；与此同时，`Channels` 与 `Trace` 右侧工作区也同步压回更克制的一套页头、分区卡和空态/错误态表面语气，继续统一三类控制面页面的设计语言而不打散既有信息架构。
