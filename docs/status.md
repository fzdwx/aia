...
- 最新 `Question` runtime 收口：`agent-runtime` 现已把 `Question` 从普通 runtime tool 回显推进到真正的最小等待态——工具 definition 已独立出 `tape_tools.rs`，模型只需传 `questions`，`request_id/turn_id` 由 runtime 自行生成；completion 处理中一旦命中 `Question`，runtime 会写入 `tool_call(Question)` + `question_requested`，并以显式 `TurnOutcome::WaitingForQuestion` / `turn_waiting_for_question` 提前结束本轮，server/runtime worker 与 tape 快照恢复也已开始承接该等待态。与此同时，共享 `ToolArgsSchema` 已补齐嵌套 struct / `Vec<struct>` 支持，`Question` definition 不再需要手写裸 JSON schema。
- 最新 `Question` 恢复闭环：`PUT /api/session/question` 当前不再只是把 `question_resolved + tool_result(Question)` 追加到 tape；`apps/agent-server` 在校验并落盘回答后，会直接把更新后的 tape 回灌到 runtime，并通过新的恢复入口让 `agent-runtime` 从原 `turn_id` 的 `turn_waiting_for_question` 现场重建 buffer、把结构化 `QuestionResult` 当作同一轮里的工具结果继续送回模型，因此“等待回答 → 用户提交 → 原 turn 继续执行”这条 RFC 主链现在已经真正闭环，而不是停留在 pending UI 与持久化层面。对应回归已覆盖 runtime 续跑和 server 路由恢复两条路径。
