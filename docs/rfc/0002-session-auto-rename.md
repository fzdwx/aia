...
AI 输出后的共享规范化规则仍然需要保留，建议包括：

1. 去除前后空白与多余换行
2. 将连续空白压成单个空格
3. 去除开头结尾明显无信息量的标记，如引号、纯标点
4. 对超长内容截断到合适长度
5. 拒绝模型输出解释性前缀，例如“这个 session 可以命名为：”
6. 对明显无意义标题跳过，例如只剩 `你好`、`继续`、`帮我看看` 这类泛化程度过高的短语
...
- 最短阈值：清洗后至少有可辨识主题
- 常规目标：约 `12-28` 个中英混合字符的可读标题
- 超长时截断并保留完整词边界的优先级高于死板的字节切分

本 RFC 不强制具体字符数常量，但要求标题生成 helper 在共享层保持可测试、可调整，而不是散落在 handler 或 UI 里。
...
- 列表主行显示标题
- 次行或尾部弱化显示相对时间，例如 `刚刚`、`5 分钟前`、`2 小时前`、`昨天`
...
- `normalize_generated_session_title(...)`：规范化 AI 输出，失败时返回 `None`

### Rust: 自动重命名 prompt 契约建议

为了避免提示词在实现期变成随手拼接字符串，建议把 session 自动重命名 prompt 也收口成共享 helper，例如：

```rust
pub struct SessionRenamePromptInput {
    pub current_title: String,
    pub recent_user_turns: Vec<String>,
    pub recent_assistant_summaries: Vec<String>,
}

pub fn build_session_rename_prompt(input: &SessionRenamePromptInput) -> String;
```

建议 prompt 约束至少包含：

- 你要为当前 session 生成一个极短标题
- 标题必须概括最近几轮对话的共同主题
- 不要输出解释、引号、编号、markdown
- 直接输出标题本身
- 尽量控制在一个短短的自然语言标题长度内
