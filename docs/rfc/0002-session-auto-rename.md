...
截至当前代码，RFC 头部的 `Implemented` 结论成立。当前真实实现可概括为：

1. `SessionRecord` / SQLite 已包含 `title_source`、`auto_rename_policy`、`last_active_at`，以及自动重命名计数与阈值字段
2. turn 完成后会更新 `last_active_at`，并在满足条件时通过 `SessionAutoRenameService` 异步调度标题生成
3. 标题生成请求带独立 trace kind：`session_rename`
4. 重命名成功后通过 `session_updated` 向客户端投影；Web 侧已实现标题动画与最近活跃时间展示
5. channel 创建的 session 当前默认显式写入 `SessionAutoRenamePolicy::Disabled`

阅读下文时要注意：正文中有些段落写的是“建议”“推荐”“可选实现”，它们主要用来解释设计意图；当前代码事实请优先看：

- `crates/agent-store/src/session.rs`
- `apps/agent-server/src/session_manager/auto_rename.rs`
- `apps/agent-server/src/session_manager/turn_execution.rs`
...
> 说明：这里的“随机 / 抖动”在当前实现里是一个非常轻量的阈值生成策略，不是复杂概率模型；当前代码只保证行为大体落在 `3~5` 次对话窗口内。
...
> 历史说明：这一节里的“建议 / 推荐”大部分已经在当前代码落地；仍保留该表述主要是为了说明设计原因，而不是表示这件事尚未开始实现。
