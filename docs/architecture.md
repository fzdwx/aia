...
- 通过后台 runtime worker 独占 `AgentRuntime`、provider registry 与 session 落盘状态，HTTP 路由通过消息传递访问运行时
- provider 当前信息与 provider 列表通过共享快照读取，避免长时间 turn 把所有路由一起锁住
...
- `POST /api/turn` 仍保持 fire-and-forget，但真正的 turn 执行、事件回收与 session 保存都在 worker 内串行完成
