...
- trace 诊断读路径又进一步拆成“单次 overview 读取 + 明确过滤”的控制面：`apps/agent-server` 继续提供按 `request_kind` 过滤的 `/api/traces/overview` 供 explorer 读取 loop 列表，同时新增 `/api/traces/dashboard` 作为 overview analytics 入口；`agent-store` 在 span 入库时同步维护 `llm_trace_loops` 聚合表，直接把同一 agent loop 的 LLM span / tool span 收敛为稳定 loop 记录，因此 overview 的 `page.items` 天然按 agent loop 分页，不再在查询期把单个模型调用临时拼装成 loop。与此同时，runtime 生成的 `session_id` 现在会随 trace 一起落盘，`llm_trace_loops` 也进一步承接每 loop 的 `session_id`、估算成本与代码增删行；为了避免 overview analytics 每次读取都重刷整张 loop 表，store 额外维护 `llm_trace_dirty_loops` 脏队列，普通写路径在同事务里刷新受影响 loop 并清掉 dirty 标记，dashboard 读路径只对脏 trace 与 legacy 空 `session_id` loop 做 reconciliation，再按时间窗聚合出失败/部分失败计数、`input/output/cache` token 趋势和年度活跃热力图。原有 summary 仍以 `llm_trace_overview_summaries` 作为稳定读快照，写路径维持“先重建受影响 loop，再按 loop 旧值/新值差量更新 summary”；`unique_models` 与 `p95_duration_ms` 继续由 `llm_trace_summary_model_counts`、`llm_trace_summary_duration_buckets` 两张辅助聚合表维持精确值，避免每次 span 入库都重扫整张 `llm_trace_loops`
