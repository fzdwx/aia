...
以下是按优先级排列的改进方向，每次选择一个推进：

### Tier 1：基础完善
- [ ] MCP 工具集成到 runtime tool registry
- [ ] 为 builtin-tools 补充边界测试（大文件、二进制文件、权限错误）
- [ ] trace 数据分页查询 + 按 session/turn 过滤
- [ ] 优化上下文压缩 prompt，提高摘要质量
- [ ] session 恢复机制——server 重启后恢复进行中的 turn

### Tier 2：能力增强
- [ ] 子 agent 支持——在 runtime 层支持 fork+并发执行
- [ ] 工具超时与取消机制
- [ ] 多模态支持——图片输入传递给模型
- [ ] 结构化输出——支持 JSON schema 约束模型输出
- [ ] 工具审批机制——高危操作需用户确认

### Tier 3：智能提升
- [ ] 动态 system prompt——根据工作区类型（Rust/JS/Python）调整指令
- [ ] 工具使用反馈循环——追踪工具成功率，调整策略
- [ ] 多步规划——复杂任务分解为子目标
- [ ] 错误恢复策略——模型返回错误时的自动重试/回退
- [ ] 上下文优先级——重要信息在压缩时保留

### Tier 4：生态
- [ ] CLI 客户端（非 server 模式）
- [ ] 插件系统——用户自定义工具的标准接口
- [ ] OTel 导出——trace 数据发送到外部可观测性平台
- [ ] 多 session 并发——支持多个独立会话同时运行
- [ ] 团队协作——共享 session 和 provider 配置
