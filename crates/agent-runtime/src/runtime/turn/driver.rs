    ...
    fn restore_waiting_turn(
        ...
        for entry in entries {
            ...
            if let Some(result) = entry.as_tool_result() {
                ...
                blocks.push(crate::TurnBlock::ToolInvocation { invocation: Box::new(invocation.clone()) });
                tool_invocations.push(invocation);
            }
        }

        let user_message =
            user_message.ok_or_else(|| RuntimeError::session("missing user message for waiting turn"))?;
